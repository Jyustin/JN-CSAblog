...
public class LinearSearchex {
    ...
    // Iterative implementation for ArrayList<Integer>
    public static int iterativeLinearSearch(ArrayList<Integer> list, int target) {
        // Start from the first element of the list
        for (int i = 0; i < list.size(); i++) {
            // Compare each element with the target element until a match is found or the end of the list is reached
            if (list.get(i).equals(target)) {
    ...
    public static void main(String[] args) {
        ...
        int index = iterativeLinearSearch(myList, 20);
        if (index != -1) {
            System.out.println("Element found at index: " + index);
        } else {
            System.out.println("Element not found.");
...
ANS: if the list is sorted already and we want an element at start or end.
...
ANS: if we are sorting through node trees and are trying to go through the entire node tree with a function which analyzes the node tree we would
want to use recursion to rerun the function again for each node we are looking at.
...
1. the 2 different forms can be more efficient depending on the data. if the data is sorted and the data is very large, then binary search would be more efficient.
However, Linear search can be more effective with smaller, unsorted data sets.

2. You won't be able to use binary search if the dataset is not already sorted.

3. 2, since it checks for success and sets it as boolean which returns true or false at the end if the value is detected.
