...
could fail because we can't add 26.2 due to it not being a string.
...
the names of people who must be killed, and only printing out names that start with "police:". 
...
that can be set to minimum if one is found.
