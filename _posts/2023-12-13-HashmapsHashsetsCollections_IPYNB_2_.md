...
ANS: create a list of items in a warehouse.

Could also use key-pair values to create objects in a store with price as value and key is item.
...
public class ShoePrices {
    public static void main(String[] args) {
        ...
        // If "Nike" key does not exist, create a new list and add the value.
        hashMap.computeIfAbsent("Nike", k -> new ArrayList<>()).add(41.97);
        hashMap.computeIfAbsent("Nike", k -> new ArrayList<>()).add(80.97);

        hashMap.put("Adidas", List.of(69.99));
        hashMap.put("Vans", List.of(55.00));
...
findbyemail lets us fetch persons by email 

findallbyorderbynameasc is finding by names, by ascending order

findByNameContainingIgnoreCaseOrEmailContainingIgnoreCase is finding by names or emails, ignoring specific cases

findByEmailAndPassword finds by email or by password
...
ANS: the personrole are essentially attributes associated with persons, however these are stored in a different table which can be combined with the person table to create a many to many relationship. This relates to collections in that personroles can be thought of as pairs with the person_id key values, and vice versa, as roles can be used to find persons.
...
tutorial_id is in the joincolumn of the note table. this represents the 1 entity that the multiple persons are associated with.
...
edited person.db stats section, which uses JSONB column definition
